...
class GenerateForm extends React.Component {
  ...
  getFields = () => {
    ...
    elements.map((item, i) => {
      ...
      if(itemProps.supportManualOnFormChange) {
        itemProps.onFormChanged = this.onChange;
        delete itemProps.supportManualOnFormChange;
      }
      if (!item.name || item.type === 'view') {
        children.push(<item.component key={uuid()}
                                      size={'large'} {...itemProps} />);
        return;
      }
